...
import { Calculator, Banknote, ShoppingBag, Coins, Wallet, Sparkles, ArrowRight, Download, Users, Plus, Trash2, Package, X, PieChart, ShieldCheck, Home, MinusCircle, Clock, Briefcase, Save, FileText, Search, ChevronRight, ListChecks } from 'lucide-react';
...
const App = () => {
  ...
  const [appVersion] = useState("v4.1 (Salvataggio Sicuro)"); 
  
  const [currentView, setCurrentView] = useState('menu'); 
  const [currentDate, setCurrentDate] = useState(new Date().toISOString().slice(0,10));
  
  // STATO SALVATAGGIO (Nuovo!)
  ...
  useEffect(() => {
    ...
    const unsubInvestors = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'investors', shopId), (snap) => {
        if (snap.exists()) setInvestors(snap.data().list || []);
    });
  ...
  const saveList = async (list, context) => {
      ...
      try {
        ...
        setTimeout(() => setSaveStatus("saved"), 800);
      } catch (err) { setSaveStatus("error"); alert("Errore salvataggio lista!"); }
  };
  ...
  // UTILS
  const formatEUR = (v) => new Intl.NumberFormat('it-IT', { style: 'currency', currency: 'EUR' }).format(v || 0);
  const formatNum = (v) => new Intl.NumberFormat('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(v || 0);
  const getDaysInactive = (str) => { try { return Math.ceil(Math.abs(new Date() - new Date(str)) / (86400000)); } catch(e) { return 0; } };
  ...
  const generateDebtorsPDF = () => {
    ...
    const tableColumn = ["Nome", "Data", "Note", "Importo (€)"];
    const tableRows = [];
    let total = 0;
    [...list].sort((a, b) => a.name.localeCompare(b.name)).forEach(item => {
        tableRows.push([item.name, new Date(item.date).toLocaleDateString('it-IT'), item.note || "-", formatEUR(item.amount)]);
        total += parseFloat(item.amount) || 0;
    });
    tableRows.push(["", "", "TOTALE:", formatEUR(total)]);
    autoTable(doc, { head: [tableColumn], body: tableRows, startY: 45, headStyles: { fillColor: color } });
    doc.save(`${title}_${shopId}.pdf`);
  };

  const downloadBackup = (type) => {
    const list = type === 'debtors' ? debtors : type === 'investors' ? investors : globalCatalog;
    if (list.length === 0) { alert("Nessun dato."); return; }
    const blob = new Blob([JSON.stringify(list, null, 2)], { type: "application/json" });
    const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `BACKUP_${type}_${shopId}.json`;
    document.body.appendChild(link); link.click(); document.body.removeChild(link);
  };
  
  const generateMonthlyReport = async () => {
    setLoadingReport(true); setShowReportModal(true);
    const prefix = `${shopId}_${currentDate.slice(0, 7)}`;
    try {
        const snap = await getDocs(collection(db, 'artifacts', appId, 'public', 'data', 'daily_sheets'));
        let tDiff = 0, tAgio = 0, dData = [];
        snap.forEach((doc) => {
            if (doc.id.startsWith(prefix)) {
                const d = doc.data();
                const da = d.data || {};
                const agio = (d.products||[]).reduce((acc, p) => acc + (parseFloat(p.agio||0)*parseFloat(p.qty||0)), 0);
                const accP = (d.products||[]).reduce((acc, p) => acc + (parseFloat(p.price||0)*parseFloat(p.qty||0)), 0);
                const real = Object.keys(d.cashBreakdown||{}).reduce((acc, k) => acc + (parseFloat(k)*d.cashBreakdown[k]), 0) + (da.totaleMonete||0) + (da.totaleAssegni||0) + (da.soldiLasciatiMattina||0);
                const theo = (da.incassoGiornaliero||0) + (da.riscossioni||0) + (da.ricariche||0) + accP - (da.crediti||0) - (da.pos||0) - (da.buoniCarburante||0);
                tDiff += (real - theo); tAgio += agio;
                dData.push({ date: doc.id.split('_').pop(), diff: (real - theo), agio });
            }
        });
        dData.sort((a, b) => a.date.localeCompare(b.date));
        setMonthlyStats({ totalDiff: tDiff, totalAgio: tAgio, daysData: dData });
    } catch (e) { console.error(e); } finally { setLoadingReport(false); }
  };

  // --- COMPONENTI UI ---
  const StatusBadge = () => {
      if(saveStatus === 'saving') return <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-1 rounded-full flex items-center gap-1 animate-pulse">💾 Salvataggio...</span>;
      if(saveStatus === 'saved') return <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full flex items-center gap-1">✅ Dati al sicuro</span>;
      if(saveStatus === 'error') return <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full flex items-center gap-1">❌ Errore conn.</span>;
      return null;
  };
  ...
  if (currentView === 'menu') return (
    <div className="min-h-screen bg-slate-100 p-4 font-sans flex flex-col items-center justify-center">
        <div className="w-full max-w-md space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4"><button onClick={() => { setActiveContext('debtors'); setCurrentView('debtors'); }} className="bg-white p-5 rounded-2xl shadow-md border-l-8 border-rose-500 flex flex-col items-center gap-2 active:scale-95 transition-transform"><div className="bg-rose-100 p-3 rounded-full"><Users className="w-6 h-6 text-rose-600" /></div><h3 className="font-bold text-slate-700">Debitori</h3></button><button onClick={() => { setActiveContext('investors'); setCurrentView('investors'); }} className="bg-white p-5 rounded-2xl shadow-md border-l-8 border-violet-500 flex flex-col items-center gap-2 active:scale-95 transition-transform"><div className="bg-violet-100 p-3 rounded-full"><Briefcase className="w-6 h-6 text-violet-600" /></div><h3 className="font-bold text-slate-700">Investitori</h3></button></div>
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20 md:pb-0"> 
      <div className="max-w-7xl mx-auto md:p-6 p-2">
        <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 mb-4 flex flex-col gap-3 sticky top-0 z-30">
            <div className="flex justify-between items-center"><div className="flex items-center gap-2"><button onClick={() => setCurrentView('menu')} className="p-2 bg-indigo-50 text-indigo-600 rounded-lg mr-1"><ArrowLeft size={18}/></button><StatusBadge/></div><div className="flex gap-2"><button onClick={downloadCSV} className="p-2 bg-slate-100 rounded-lg text-slate-600"><Download size={18}/></button></div></div>
