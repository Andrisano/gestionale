...
import { Calculator, Banknote, ShoppingBag, Coins, Wallet, Sparkles, ArrowRight, ArrowLeft, Download, Calendar, Users, Plus, Trash2, Package, X, PieChart, ShieldCheck, Home, MinusCircle, Clock, Briefcase, Save, Search, ListChecks, ChevronRight, RefreshCcw, StickyNote, Box, FileText, Fuel, AlertTriangle } from 'lucide-react';
...
const App = () => {
  ...
  const [appVersion] = useState("v7.0 (Full)"); 
  ...
  // --- AUTH ---
  useEffect(() => {
    signInAnonymously(auth).catch(err => console.error("Login err:", err));
    onAuthStateChanged(auth, (u) => { setUser(u); setConnectionStatus(u ? "connected" : "disconnected"); });
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubDebtors = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'debtors', shopId), (snap) => {
        if (snap.exists()) { const list = snap.data().list; setDebtors(Array.isArray(list) ? list : []); } 
    });
    const unsubInvestors = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'investors', shopId), (snap) => {
        if (snap.exists()) { const list = snap.data().list; setInvestors(Array.isArray(list) ? list : []); } 
    });
    
    const unsubDaily = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'daily_sheets', `${shopId}_${currentDate}`), (docSnap) => {
      if (docSnap.exists()) {
  ...
  const saveToCloud = async (newData, newBreakdown, newProducts) => {
    ...
    try {
      await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'daily_sheets', `${shopId}_${currentDate}`), {
        data: newData, cashBreakdown: newBreakdown, products: newProducts,
        lastUpdated: new Date().toISOString(), updatedBy: user.uid
      }, { merge: true });
      setTimeout(() => setSaveStatus("saved"), 500);
    } catch (err) { setSaveStatus("error"); alert(err.message); }
  };
  ...
  const saveList = async (list, context) => {
      ...
      setSaveStatus("saving");
      try {
          await setDoc(doc(db, 'artifacts', appId, 'public', 'data', context, shopId), { list: Array.isArray(list) ? list : [] }, { merge: true });
          setTimeout(() => setSaveStatus("saved"), 500);
      } catch (err) { setSaveStatus("error"); }
  };
  ...
  const addProductToDay = (catalogItem = null) => {
    const newProduct = catalogItem ? { ...catalogItem, id: Date.now(), qty: 1 } : { id: Date.now(), name: '', price: 0, agio: 0, qty: 1 }; 
    const newProds = [...products, newProduct]; setProducts(newProds); saveToCloud(data, cashBreakdown, newProds);
  };
  ...
  const removeProductDay = (id) => {
    const newProds = products.filter(p => p.id !== id); setProducts(newProds); saveToCloud(data, cashBreakdown, newProds);
  };

  const addNewInventoryItem = () => {
      if(!newItemName) return;
      const price = parseFloat(newItemPrice.replace(',','.')) || 0; const agio = parseFloat(newItemAgio.replace(',','.')) || 0; const stock = parseInt(newItemStock) || 0;
      const newCatalog = [...globalCatalog, { id: Date.now(), name: newItemName, price, agio, stock }];
  ...
  const updateInventoryStock = (id, delta) => {
      updateGlobalCatalog(globalCatalog.map(i => i.id === id ? { ...i, stock: Math.max(0, (i.stock || 0) + delta) } : i));
  };
  const removeInventoryItem = (id) => { if(confirm("Eliminare?")) updateGlobalCatalog(globalCatalog.filter(i => i.id !== id)); };

  const openAddEntityModal = (context) => { setActiveContext(context); setNewEntityName(""); setNewEntityAmount(""); setNewEntityDate(new Date().toISOString().slice(0,10)); setNewEntityNote(""); setShowAddEntityModal(true); };
  const confirmAddEntity = () => {
    if (!newEntityName.trim()) return;
    const amount = parseFloat(newEntityAmount.toString().replace(',', '.')) || 0;
  ...
  const confirmTransaction = () => {
     const val = parseFloat(transactionData.amount.toString().replace(',', '.'));
     if (isNaN(val) || val <= 0) return;
     const list = activeContext === 'debtors' ? debtors : investors;
     const updatedList = list.map(d => {
         if (d.id === transactionData.id) {
             const transaction = { id: Date.now(), date: new Date().toISOString().slice(0,10), amount: val, note: transactionData.type === 'add' ? 'Aggiunta' : 'Pagamento', type: transactionData.type };
  ...
  const removeEntity = (id, context) => { 
      ...
      saveList(list.filter(d => d.id !== id), context);
  };
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            if (doc.id.startsWith(currentMonthPrefix)) {
                ...
                const diff = (Object.keys(dayB).reduce((acc, k) => acc + (parseFloat(k)*dayB[k]), 0) + (dayD.totaleMonete||0) + (dayD.totaleAssegni||0) + (dayD.soldiLasciatiMattina||0)) - ((dayD.incassoGiornaliero||0) + (dayD.riscossioni||0) + (dayD.ricariche||0) + (dayP.reduce((acc, p) => acc + (parseFloat(p.price||0)*parseFloat(p.qty||0)), 0)) - (dayD.crediti||0) - (dayD.pos||0) - (dayD.buoniCarburante||0));
  ...
  const generateDebtorsPDF = () => {
    ...
    let total = 0;
    list.forEach(item => {
        rows.push([item.name, new Date(item.date).toLocaleDateString('it-IT'), item.note || "-", formatEUR(item.amount)]);
        total += parseFloat(item.amount) || 0;
    });
    rows.push(["", "", "TOTALE:", formatEUR(total)]);
    autoTable(doc, { head: [["Nome", "Data", "Note", "Importo"]], body: rows, startY: 45, headStyles: { fillColor: color } });
  ...
  const downloadBackup = (type) => {
    let list = [];
    let filename = "";
    if (type === 'debtors') { list = debtors; filename = `BACKUP_DEBITORI_${shopId}.json`; } 
    else if (type === 'investors') { list = investors; filename = `BACKUP_INVESTITORI_${shopId}.json`; }
    else if (type === 'inventory') { list = globalCatalog; filename = `BACKUP_INVENTARIO_${shopId}.json`; }
    if (list.length === 0) { alert("Nessun dato."); return; }
  ...
  useEffect(() => {
    ...
    const handlePopState = (event) => { if (event.state && event.state.view) setCurrentView(event.state.view); else setCurrentView('menu'); };
    window.addEventListener('popstate', handlePopState);
    return () => window.removeEventListener('popstate', handlePopState);
  }, []);
  const navigateTo = (viewName) => { window.history.pushState({ view: viewName }, null, ''); setCurrentView(viewName); };
  const goBack = () => window.history.back();

  const StatusBadge = () => {
      if(saveStatus === 'saving') return <span className="text-[10px] bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full font-bold animate-pulse flex items-center gap-1"><Save size={10}/> Saving...</span>;
      if(saveStatus === 'saved') return <span className="text-[10px] bg-green-100 text-green-800 px-2 py-1 rounded-full font-bold flex items-center gap-1"><CheckCircle size={10}/> Saved</span>;
      if(saveStatus === 'error') return <span className="text-[10px] bg-red-100 text-red-800 px-2 py-1 rounded-full font-bold flex items-center gap-1"><AlertCircle size={10}/> Error</span>;
      return null;
  };
      ...
      return (
        <div className="min-h-screen bg-slate-50 font-sans p-4 pb-20">
            <div className="max-w-2xl mx-auto">
                <div className="bg-white p-4 rounded-xl shadow-sm mb-4 sticky top-0 z-20 border-b border-slate-100"><div className="flex justify-between items-center mb-3"><div className="flex items-center gap-2"><button onClick={goBack} className="p-2 bg-slate-100 rounded-lg text-slate-600"><Home size={20}/></button><StatusBadge/></div><h2 className="font-bold text-lg text-slate-700">{isDebtors ? 'Registro Debiti' : 'Registro Investitori'}</h2><div className="flex gap-2"><button onClick={() => downloadBackup(context)} className="p-2 bg-slate-100 text-slate-600 rounded-lg" title="Backup JSON"><Save size={20}/></button><button onClick={generateDebtorsPDF} className="p-2 bg-red-100 text-red-600 rounded-lg" title="PDF"><FileText size={20}/></button><button onClick={() => openAddEntityModal(context)} className={`p-2 bg-${ThemeColor}-100 text-${ThemeColor}-700 rounded-lg`}><Plus size={20}/></button></div></div><div className="relative"><Search className="absolute left-3 top-2.5 text-slate-400" size={18}/><input type="text" placeholder="Cerca..." className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:border-slate-400" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/></div></div>
  ...
  if (currentView === 'menu') return (
    <div className="min-h-screen bg-slate-100 p-4 font-sans flex flex-col items-center justify-center">
        <div className="w-full max-w-md space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4"><button onClick={() => { setActiveContext('debtors'); navigateTo('debtors'); }} className="bg-white p-5 rounded-2xl shadow-md border-l-8 border-rose-500 flex flex-col items-center gap-2 active:scale-95 transition-transform"><div className="bg-rose-100 p-3 rounded-full"><Users className="w-6 h-6 text-rose-600" /></div><h3 className="font-bold text-slate-700">Debitori</h3></button><button onClick={() => { setActiveContext('investors'); navigateTo('investors'); }} className="bg-white p-5 rounded-2xl shadow-md border-l-8 border-violet-500 flex flex-col items-center gap-2 active:scale-95 transition-transform"><div className="bg-violet-100 p-3 rounded-full"><Briefcase className="w-6 h-6 text-violet-600" /></div><h3 className="font-bold text-slate-700">Investitori</h3></button></div>
  ...
  if (currentView === 'inventory') return (
    <div className="min-h-screen bg-slate-50 font-sans p-4 pb-20">
       <div className="max-w-2xl mx-auto">
          <div className="bg-white p-4 rounded-xl shadow-sm mb-4 sticky top-0 z-20 flex justify-between items-center border-b border-slate-100"><button onClick={goBack} className="p-2 bg-slate-100 rounded-lg text-slate-600"><Home size={20}/></button><h2 className="font-bold text-lg text-slate-700">Magazzino</h2><div className="flex gap-2"><button onClick={() => downloadBackup('inventory')} className="p-2 bg-slate-100 text-slate-600 rounded-lg"><Save size={20}/></button><button onClick={() => setShowInventoryModal(true)} className="p-2 bg-amber-100 text-amber-700 rounded-lg"><Plus size={20}/></button></div></div>
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20 md:pb-0"> 
      <div className="max-w-7xl mx-auto md:p-6 p-2">
        ...
        <div className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 mb-4 flex flex-col gap-3 sticky top-0 z-30">
            <div className="flex justify-between items-center"><div className="flex items-center gap-2"><button onClick={goBack} className="p-2 bg-indigo-50 text-indigo-600 rounded-lg mr-1"><ArrowLeft size={18}/></button><StatusBadge/></div><div className="flex gap-2"><button onClick={downloadCSV} className="p-2 bg-slate-100 rounded-lg text-slate-600"><Download size={18}/></button></div></div>
